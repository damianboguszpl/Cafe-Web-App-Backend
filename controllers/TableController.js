...
module.exports = {
    create: async (req, res) => {
        if (!req?.body?.numberOfSeats) return res.status(400).json({ 'message': 'Nie podano liczby miejsc.' });
        if (!req?.body?.number) return res.status(400).json({ 'message': 'Nie podano numeru Stolika.' });
        if (!req?.body?.TableStatusId) return res.status(400).json({ 'message': 'Nie podano Id Statusu Stolika.' });
        ...
        return res.status(201).json({'message': `Dodano nowy Stolik.`, 'data': newTable});
    },
    ...
    update: async (req, res) => {
        ...
        return res.json({'message': `Zaktualizowano Stolik.`});
    },
    ...
    delete: async (req, res) => {
        ...
        return res.json({'message': `Usunięto Stolik.`});
    },
